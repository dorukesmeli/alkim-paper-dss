...
def solve_model2_heuristic(data: Dict, verbose: bool = True):
    ...
    while j_idx < ns:
        ...
        for t in T:
            ...
            # o type için demand var mı?
            type_available = any(
                remaining[d] > 0 and TD[d] == t for d in D
            )
            if not type_available:
                continue

            pool = build_candidate_pool(
                D, TD, LD, remaining, t, L, B
            )
            ...
            for p in pool:
                ...
                score, tw, pw, mult = pattern_score(
                    p, prev_used_pattern, remaining, L, ns, lam
                )
                ...
                if score < best_score_val:
                    ...
                    if mult > best_mult:
                        ...
                        best_mult = mult
                        best_pw = pw
                    elif mult == best_mult and pw < best_pw:
                        best_pattern = p
                        best_type = t
                        best_mult = mult
                        best_pw = pw

        # ❌ hiç pattern yoksa → reel boş
        if best_pattern is None:
            reel_plans.append(
                ReelPlan(reel_id=reel_id, reel_type=None, used=False, pattern=None)
            )
            j_idx += 1
            continue
        ...
        applied = 0
        while applied < feasible_mult and j_idx < ns:
            ...
            reel_plans.append(
                ReelPlan(
                    reel_id=rid,
                    reel_type=best_type,   # 🔥 artık decision
                    used=True,
...
if __name__ == "__main__":
    ...
    solve_model2_heuristic(data_model2_mixed20_pw80, verbose=True)

    end_time = time.time()
    elapsed = end_time - start_time

    print("\n⏱️ Heuristic Model 2 Süresi:")
